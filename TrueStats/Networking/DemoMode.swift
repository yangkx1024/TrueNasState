...
/// The fixed dashboard the demo account shows.
struct DemoSnapshot {
...
///
/// This type only produces values; the view model applies them. Writing straight into
/// the view model is what forced every piece of dashboard state to be publicly settable.
enum DemoMode {
    ...
    static func snapshot() -> DemoSnapshot {
        DemoSnapshot(
    ...
    /// One step of the fake realtime walk, so the demo dashboard visibly ticks.
    /// Values stay inside plausible bounds rather than drifting off the gauge.
    static func nextStats(after current: RealtimeStats?) -> RealtimeStats {
        let cpu = min(85, max(2, (current?.cpuUsagePercent ?? 12) + Double.random(in: -4...4)))
        let memoryTotal = current?.memoryTotalBytes ?? (32 * 1024 * 1024 * 1024)
        let memoryUsed = min(memoryTotal, max(0,
            (current?.memoryUsedBytes ?? 8 * 1024 * 1024 * 1024)
                + Int64.random(in: -200_000_000...200_000_000)))
        var next = current ?? RealtimeStats()
